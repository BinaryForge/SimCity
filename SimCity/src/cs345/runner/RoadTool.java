...
/**
 * Tool for creating a road for the region in the grid.
 *
 * This tool can be used for creating roads in multiple regions.
 *
 * @author Chris Reedy (Chris.Reedy@wwu.edu)
 */
public class RoadTool extends GridTool {

   public static final String ROAD_COLOR = "lightgrey";
   ...
   // private GridLocation priorLocCell = null;

   private Color toolColor = Color.web(ROAD_COLOR, 0.5);
   private Color strokeColor = Color.web(ROAD_COLOR, 0.75);

    /**
     * Construct a Road tool.
     * @param runner the associated GuiRunner for this tool
     * @param display the GridDisplay object for this tool
     */
   RoadTool(GuiRunner runner, GridDisplay display) {
   ...
   private void displayRect(GridLocation loc) {
      if (inDisplay && !loc.equals(priorLoc)) {
         if (pressedLoc == null) {
           ...
           /*Conditional statememt for the snap action of the displayRect
            *when user is selecting the area for road.
            */
            if(width > height){                                         
               display.setToolRect(toolRect, rectX, rectY, width, 1);   
            }
            else{                                                      
               display.setToolRect(toolRect, rectX, rectY, 1, height);
            }
         }
         priorLoc = loc;
      }
   ...
   @Override
     ...
     * a roadCommand is created for the resulting rectangle. The rectangle
   ...
   @Override
    public void onMouseReleased(GridLocation loc) {
      ...
      if (loc != null && pressedLoc != null) {
         ...
         int width = Math.abs(loc.x - pressedLoc.x) + 1;
         int height = Math.abs(loc.y - pressedLoc.y) + 1;   
         Command cmd = new RoadCommand(runner,             //Creates a new RoadCommand after mouse
                    new GridRectangle(rectX, rectY, width, height));    //has been released.
